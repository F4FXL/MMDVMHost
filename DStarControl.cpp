...
bool CDStarControl::writeModem(unsigned char *data, unsigned int len)
{
	...
	if (type == TAG_HEADER) {
		...
		if (::memcmp(data + 1U + DSTAR_VOICE_FRAME_LENGTH_BYTES, DSTAR_SYNC_BYTES, DSTAR_DATA_FRAME_LENGTH_BYTES) == 0) {
			m_rfLookBackSlowData.start();
			m_rfN = 0U;
		} else {
			m_rfLookBackSlowData.add(data + 1U);
		}
		...
		if(m_rfDataLookBackIndex == 0U) {
			...
			if (m_rfState == RPT_RF_STATE::AUDIO){
				unsigned char type = m_rfLookBackSlowData.getType(true);
				...
				if ((type & DSTAR_SLOW_DATA_TYPE_FASTDATA_MASK) == DSTAR_SLOW_DATA_TYPE_FASTDATA && type >= 0x81U && type <= 0x9CU){
